...
const ManageProduct = () => {
    ...
    const handlePageChange = (pageNumber)=>{
        ...
        const handleDeleteProduct = async (id) => {
            try {
                // Pass the product ID as an object with the 'id' key
                const response = await deleteProduct({ id }).unwrap();
      ...
      <div className="block w-full overflow-x-auto">
        <table className="items-center bg-transparent w-full border-collapse">
          ...
          { 
              ...
              <td className="border-t-0 px-6 align-middle border-l-0 hover:text-primary hover:underline cursor-pointer border-r-0 text-xs whitespace-nowrap p-4">
          ...
          <div className='mt-6 flex items-center justify-center'>
            ...
            className='px-4 py-2 bg-gray-300 text-gray-700 rounded-md mr-2'>Previous</button>
            {
                [...Array(totalPages)].map((_,index)=>(
                    ...
                    className={`px-4 py-2 ${currentPage === index+1 ? 'bg-blue-500 text-white' : 'bg-gray-300 text-white' } rounded-md mx-1`}>{index+1}</button>
